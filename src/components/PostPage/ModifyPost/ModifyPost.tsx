...
import * as St from './STModifyPost'
...
const ModifyPost: React.FC = () => {
  ...
  const writePostAPI = {
    ModifyPostApi : () => customAxios.post(''),            // 게시글 수정
  }
  ...
  //     } catch (error) {
  //       console.error('수정된 게시글 등록 실패', error);
  //       throw error;
  //     }
  //   }
  // );
  ...
  const postAdd = async () => {
    ...
    //   navigate('/');
    // } catch (error) {
    //   console.log('수정된 게시글 등록 실패', error);
    // }
  }
    ...
    <>
      <St.PostSection>
        ...
        <St.ButtonWrap>
          ...
          <Button onClick={postAdd}>수정</Button>
        </St.ButtonWrap>
      </St.PostSection>
    </>
  )
